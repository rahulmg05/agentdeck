...
never from comparing `bb_ts` — courier-measured timestamps can occasionally
...
fall back to a `bb_ts` delta, clamped at zero, if `duration_ms` is missing.
...
def _fallback_duration_ms(call: ToolCall, post_bb_ts: float) -> float:
    return max(0.0, (post_bb_ts - call.pre_event.bb_ts) * 1000)
    ...
    def observe(self, event: Event) -> ToolCall | list[ToolCall] | None:
        ...
        if name in ("PostToolUse", "PostToolUseFailure") and event.tool_use_id:
            ...
            call.duration_ms = (
                ...
                else _fallback_duration_ms(call, event.bb_ts)
            )
            call.post_bb_ts = event.bb_ts
        ...
        if name == "PostToolBatch":
            ...
            for tc in event.raw.get("tool_calls") or []:
                ...
                call.duration_ms = _fallback_duration_ms(call, event.bb_ts)
                call.post_bb_ts = event.bb_ts
